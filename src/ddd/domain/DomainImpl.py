...
class CreateMethodDefaultAPI:
    ...
    def delete_not_key(config: CodeConfig):
        ...
        class Body(JavaCode.FunctionBody):
            def function_body(self, parameter: list[Attribute]):
                self.line(f'boolean b = false;')
                self.line(f'b = {config.module.repository.low_name()}.{config.createConfig.methodName.get(1)}{config.className}({parameter[0].name});')
                self.line(f'return b;')
...
class CreateMethodExtraAPI:
    ...
    def delete(config: CodeConfig, extra: str):
        ...
        class Body(JavaCode.FunctionBody):
            def function_body(self, parameter: list[Attribute]):
                self.line(f'boolean b = false;')
                self.line(f'b = {config.module.repository.low_name()}.{config.createConfig.methodName.get(1)}{config.className}({parameter[0]});')
                self.line(f'return b;')
        function.add_body(Body())
        return function
