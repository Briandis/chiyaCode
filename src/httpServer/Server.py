...
from src.httpServer.HttpEntity import TimeMap
...
class Server:
    ...
    def __init__(self, path, listen_size=10, address=("", 11451)):
        ...
        self.path = path
        # 全局Session容器
        self.all_session: TimeMap = TimeMap()
        # 全局业务容器
        self.servlet = {}
        # 全局路由
        self.index = {}
    ...
    def run(self, client: socket.socket, address):
        ...
        session = self.all_session.get(session_id)
        if session is None:
            session = {}
        ...
        if uri in self.servlet:
            try:
                ...
                response.line.code = 500
                response.write("code:500 server error")
                traceback.print_exc()
        ...
        # 如使用了session就保存起来
        if request.session_flag:
            if session_id is None:
                session_id = str(uuid.uuid4()).replace("-", "")
                self.all_session[session_id] = session
            response.set_cookie("SESSION_ID", session_id)
        client.send(response.create())
        client.close()
